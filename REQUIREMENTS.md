...
| Date | Change |
|------|--------|
| 2026-08-04 | Fixed detail page image source: the collection item's own `imageUrl` is now preferred over the `/xmlapi2/thing` `imageUrl`, so the displayed box art matches the user's edition/language. Falls back to the `/thing` image only when the collection item has no image. Delta spec: [`openspec/changes/use-collection-image-on-detail/specs/game-detail/spec.md`](openspec/changes/use-collection-image-on-detail/specs/game-detail/spec.md) |
| 2026-08-04 | Reordered game detail fields: `Plays` moved to the bottom, after `Rating` and `Rank`; `Min age` and `Language dependence` now appear before the BGG community values. Delta spec: [`openspec/changes/move-plays-field-to-bottom/specs/game-detail/spec.md`](openspec/changes/move-plays-field-to-bottom/specs/game-detail/spec.md) |
| 2026-08-03 | Extended the player count filter with a `publisher`/`recommended`/`best` segmented button. The selected mode filters by the corresponding player count range and persists with the collection view state. Clearing filters resets the slider but keeps the selected mode. Delta specs: [`openspec/changes/filter-by-player-count-mode/specs/search-and-filter/spec.md`](openspec/changes/filter-by-player-count-mode/specs/search-and-filter/spec.md), [`openspec/changes/filter-by-player-count-mode/specs/card-layout/spec.md`](openspec/changes/filter-by-player-count-mode/specs/card-layout/spec.md) |
| 2026-08-02 | Added optional BGG-recommended and BGG-best player counts on collection cards: the `/xmlapi2/thing` values are copied to `CollectionItem` during sync, stored in the Drift cache, and appended to the player count line when the user enables "Show recommended player count" and/or "Show best player count" in the card-layout settings. Delta specs: [`openspec/changes/show-bgg-recommended-best-player-counts/specs/bgg-sync/spec.md`](openspec/changes/show-bgg-recommended-best-player-counts/specs/bgg-sync/spec.md), [`openspec/changes/show-bgg-recommended-best-player-counts/specs/app-database/spec.md`](openspec/changes/show-bgg-recommended-best-player-counts/specs/app-database/spec.md), [`openspec/changes/show-bgg-recommended-best-player-counts/specs/card-layout/spec.md`](openspec/changes/show-bgg-recommended-best-player-counts/specs/card-layout/spec.md), [`openspec/changes/show-bgg-recommended-best-player-counts/specs/collection-list/spec.md`](openspec/changes/show-bgg-recommended-best-player-counts/specs/collection-list/spec.md) |
| 2026-07-24 | Added `inventoryLocation` as a configurable card field: BGG collection sync now sends `showprivate=1`, parses and trims `inventorylocation` from `<privateinfo>`, stores it in the local Drift cache, and renders it on collection cards only when a value is present. Delta specs: [`openspec/changes/add-inventory-location-field/specs/bgg-sync/spec.md`](openspec/changes/add-inventory-location-field/specs/bgg-sync/spec.md), [`openspec/changes/add-inventory-location-field/specs/card-layout/spec.md`](openspec/changes/add-inventory-location-field/specs/card-layout/spec.md), [`openspec/changes/add-inventory-location-field/specs/collection-list/spec.md`](openspec/changes/add-inventory-location-field/specs/collection-list/spec.md) |
