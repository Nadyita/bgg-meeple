...
The player count filter currently only considers the publisher-specified `minPlayers`/`maxPlayers`. A game may officially support 1–6 players, but the BGG community may recommend 3–4 and consider 4 the best. Users want to filter their collection by the community consensus rather than the publisher range. The data is already cached on collection items after the previous change, so it can be used for filtering without additional lookups.

## What Changes

Introduce a configurable player count filter mode in the collection filter panel. The mode lets users choose whether the slider filters by publisher player count, BGG-recommended player count, or BGG-best player count. The mode persists with the rest of the collection view state and is independent of the card layout settings that control whether recommended/best counts are displayed on cards.
